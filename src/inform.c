...
{
    LOG_DBG("construyendo sys_stats");
    ...
    if (sysinfo_mem(&mem) == 0) {
        ...
        LOG_DBG("sys_stats mem total=%ldkB used=%ldkB", mem.total_kb, used_kb);
        ...
        LOG_DBG("sys_stats mem FALLO — usando ceros");
...
{
    LOG_DBG("construyendo if_table (%d interfaces)", m->port_table_len);
    struct json_object *arr = json_object_new_array();

    for (int i=0; i<m->port_table_len; i++) {
        const char *ifname = m->port_table[i].ifname;
        LOG_TRACE("if_table[%d] ifname=%s", i, ifname);
        iface_stats_t stats;
        sysinfo_iface(ifname, &stats);

        LOG_TRACE("up=%d speed=%dMbps rx=%lldB tx=%lldB",
...
{
    LOG_DBG("construyendo radio_table (%d radios)", m->radio_table_len);
    struct json_object *arr = json_object_new_array();
    for (int i=0; i<m->radio_table_len; i++) {
        const uf_radio_t *r = &m->radio_table[i];
        LOG_TRACE("radio_table[%d] name=%s radio=%s ch=%d ht=%s pwr=%d he=%d",
...
{
    LOG_DBG("construyendo radio_table_stats (%d radios)", m->radio_map_len);
    struct json_object *arr = json_object_new_array();
    ...
    for (int i=0; i<m->radio_map_len; i++) {
        ...
        LOG_TRACE("radio_stats[%d] device=%s wlan=%s name=%s",
             i, rm->device, wlan_iface, radio_name);

        radio_stats_t rs;
        if (sysinfo_radio(wlan_iface,&rs)!=0) {
            LOG_DBG("sysinfo_radio FALLO en %s — usando defaults", wlan_iface);
            memset(&rs,0,sizeof(rs)); rs.noise=-95;
        }
        LOG_TRACE("radio_stats[%d] ch=%d pwr=%d cu=%d%% tx=%d%% rx=%d%% num_sta=%d noise=%d",
...
{
    LOG_DBG("construyendo port_table (%d puertos)", m->port_table_len);
    struct json_object *arr = json_object_new_array();
    for (int i=0; i<m->port_table_len; i++) {
        const uf_port_t *pt = &m->port_table[i];
        iface_stats_t stats;
        sysinfo_iface(pt->ifname, &stats);
        LOG_TRACE("port[%d] %s up=%d speed=%d poe=%d",
...
{
    LOG_DBG("construyendo eth_table (%d entradas)", m->ethernet_table_len);
    struct json_object *arr = json_object_new_array();
    for (int i=0; i<m->ethernet_table_len; i++) {
        const uf_eth_entry_t *e=&m->ethernet_table[i];
        LOG_TRACE("eth[%d] name=%s num_port=%d", i, e->name, e->num_port);
        struct json_object *o=json_object_new_object();
...
static struct json_object *build_vap_table(const uf_model_t *m)
{
    ...
    for (int i=0; i<nvaps; i++) {
        ...
        LOG_DBG("vap[%d] essid='%s' bssid=%s radio=%s name=%s",
        ...
        for (int j=0; j<m->radio_map_len; j++) {
            if (strcmp(m->radio_map[j].band,radio)==0) {
                ...
                LOG_TRACE("vap[%d] mapeado a device=%s wlan=%s canal=%d",
        ...
        LOG_DBG("vap[%d] leyendo clientes en %s (band=%s ch=%d)",
        ...
        LOG_DBG("vap[%d] '%s' — %d clientes, rx=%lldB tx=%lldB",
...
{
    LOG_DBG("build_payload — modelo=%s mac=%s ip=%s uptime=%lds adopted=%s",
    ...
    LOG_TRACE("serial = %s", mac_clean);

    char fw_version[64];
    snprintf(fw_version,sizeof(fw_version),"%s%s",m->fw_pre,m->fw_ver);
    LOG_TRACE("fw_version = %s", fw_version);
    ...
    LOG_TRACE("inform_url = %s", inform_url_buf);
    ...
    LOG_DBG("payload JSON listo — %zu bytes", json_len);
    LOG_TRACE("payload (primeros 200 chars): %.200s", s);
    char *copy = strdup(s);
...
{
    LOG_DBG("build_packet — mac=%.12s key=%.8s...", mac_hex, key_hex);

    unsigned char iv_hex[33]={0};
    if (crypto_random_hex(iv_hex,16)!=0) { LOG_DBG("random IV FALLO"); return NULL; }
    LOG_TRACE("IV generado: %s", iv_hex);

    size_t pl_len=strlen(payload);
    unsigned char *enc=malloc(pl_len+32);
    if (!enc) { LOG_DBG("build_packet OOM"); return NULL; }

    LOG_DBG("cifrando %zu bytes de payload...", pl_len);
    int enc_len=crypto_encrypt(key_hex,(char*)iv_hex,(const unsigned char*)payload,pl_len,enc);
    if (enc_len<0) { LOG_DBG("cifrado FALLO"); free(enc); return NULL; }
    LOG_DBG("cifrado OK — enc_len=%d bytes", enc_len);

    unsigned char mac_bin[6];
    crypto_hex2bin(mac_hex,mac_bin,6);

    size_t pkt_len=4+4+6+2+16+4+4+enc_len;
    LOG_DBG("construccion paquete TNBU — total=%zu bytes (4+4+6+2+16+4+4+%d)",
         pkt_len, enc_len);
    unsigned char *pkt=malloc(pkt_len);
    if (!pkt) { LOG_DBG("pkt malloc FALLO"); free(enc); return NULL; }

    unsigned char *p=pkt;
    memcpy(p,INFORM_MAGIC,4);           p+=4;  LOG_TRACE("magic='TNBU' escrito");
    put32be(p,INFORM_PKT_VERSION);      p+=4;  LOG_TRACE("pkt_ver=%u", INFORM_PKT_VERSION);
    memcpy(p,mac_bin,6);                p+=6;  LOG_TRACE("MAC escrita en cabecera");
    put16be(p,INFORM_FLAG_ENCRYPTED);   p+=2;  LOG_TRACE("flags=0x%04x (cifrado)", INFORM_FLAG_ENCRYPTED);
    unsigned char iv_bin[16];
    crypto_hex2bin((char*)iv_hex,iv_bin,16);
    memcpy(p,iv_bin,16);                p+=16; LOG_TRACE("IV (16 bytes) escrito");
    put32be(p,INFORM_DATA_VERSION);     p+=4;  LOG_TRACE("data_ver=%u", INFORM_DATA_VERSION);
    put32be(p,(uint32_t)enc_len);       p+=4;  LOG_TRACE("payload_len=%d escrito", enc_len);
    memcpy(p,enc,enc_len);
    free(enc);

    DBG_HEX("TNBU cabecera (40 bytes)", pkt, 40);
    *out_len=pkt_len;
    return pkt;
}
...
{
    LOG_DBG("parse_packet — data_len=%zu", data_len);
    if (data_len<40) { LOG_DBG("respuesta demasiado corta (%zu bytes)", data_len); return NULL; }

    if (memcmp(data,INFORM_MAGIC,4)!=0) {
        LOG_DBG("magic TNBU no encontrado en respuesta (primeros 4: %02x%02x%02x%02x)",
             data[0],data[1],data[2],data[3]);
        return NULL;
    }
    LOG_TRACE("magic TNBU OK");

    uint16_t flags    = get16be(data+14);
    uint32_t body_len = get32be(data+36);
    LOG_TRACE("respuesta flags=0x%04x body_len=%u", flags, body_len);

    if (40+body_len>data_len) { LOG_DBG("body_len=%u supera data_len=%zu", body_len,(size_t)data_len); return NULL; }

    const unsigned char *iv_bin  = data+16;
    const unsigned char *body    = data+40;

    if (flags & INFORM_FLAG_ENCRYPTED) {
        LOG_DBG("respuesta cifrada — descifrando con key=%.8s...", key_hex);
        char iv_hex[33];
        crypto_bin2hex(iv_bin,16,iv_hex);
        LOG_TRACE("IV de respuesta: %s", iv_hex);
        unsigned char *plain=malloc(body_len+1);
        if (!plain) { LOG_DBG("OOM al descifrar"); return NULL; }
        int pl=crypto_decrypt(key_hex,iv_hex,body,body_len,plain);
        if (pl<0) { LOG_DBG("descifrado FALLO"); free(plain); return NULL; }
        plain[pl]='\0';
        LOG_TRACE("descifrado OK — plain_len=%d bytes: %.200s", pl, (char*)plain);
        return (char*)plain;
    }

    LOG_DBG("respuesta sin cifrar (flags=0x%04x)", flags);
    char *copy=malloc(body_len+1);
    if (!copy) return NULL;
    memcpy(copy,body,body_len); copy[body_len]='\0';
    LOG_TRACE("respuesta plana: %.200s", copy);
    return copy;
}
                             ...
                             struct json_object *resp, char *action_out)
{
    ...
    LOG_DBG("handle_response _type='%s'", type);

    /* Volcar JSON completo de la respuesta */
    LOG_TRACE("JSON del controlador: %s", json_object_to_json_string(resp));

    if (!strcmp(type,"noop")) { strcpy(action_out,"noop"); LOG_DBG("noop recibido"); return; }

    if (!strcmp(type,"setparam")) {
        LOG_DBG("setparam recibido");
        if (json_object_object_get_ex(resp,"key",&v)) {
            const char *key=json_object_get_string(v);
            LOG_DBG("setparam key='%s'", key);
            struct json_object *val_o;
            if (json_object_object_get_ex(resp,"value",&val_o)) {
                const char *val=json_object_get_string(val_o);
                LOG_DBG("setparam value='%s'", val);
    ...
    if (!strcmp(type,"cmd")) {
        ...
        if (!strcmp(cmd,"set-adopt")||!strcmp(cmd,"adopt")) {
            ...
            LOG_INFO("REBOOT ordenado por el controlador");
            strcpy(action_out,"reboot");
            system("reboot &");
        } else if (!strcmp(cmd,"reset")) {
            LOG_INFO("RESET ordenado — borrando estado y reiniciando");
            strcpy(action_out,"reset");
            system("rm -f " OPENUF_STATE_FILE);
            system("reboot &");
        } else if (!strcmp(cmd,"locate")) {
            LOG_DBG("locate (parpadeo LED)");
            strcpy(action_out,"locate");
        } else {
            LOG_DBG("cmd desconocido '%s'", cmd);
    ...
    if (!strcmp(type,"setstate")) {
        ...
        if (rt) LOG_DBG("radio_table presente (%d entradas)", json_object_array_length(rt));
        if (vt) LOG_DBG("vap_table presente (%d entradas)", json_object_array_length(vt));
        if (rt||vt) {
            printf("[openuf] Aplicando config WiFi del controlador...\n");
            wlan_apply_config(resp,model);
        } else {
            LOG_DBG("setstate sin radio_table ni vap_table — nada que aplicar");
        }
        state_save(st);
    ...
    LOG_DBG("tipo desconocido '%s'", type);
                ...
                long uptime, char *err_out)
{
    ...
    LOG_TRACE("mac_hex=%s", mac_hex);

    LOG_DBG("construyendo JSON payload...");
    char *payload=build_payload(st,model,uptime);
    if (!payload) { strncpy(err_out,"build_payload OOM",127); LOG_DBG("payload FALLO"); return -1; }
    LOG_DBG("payload listo (%zu bytes)", strlen(payload));

    LOG_DBG("construyendo paquete TNBU...");
    size_t pkt_len=0;
    unsigned char *pkt=build_packet(mac_hex,key_hex,payload,&pkt_len);
    free(payload);
    if (!pkt) { strncpy(err_out,"build_packet failed",127); LOG_DBG("build_packet FALLO"); return -1; }
    LOG_DBG("paquete TNBU listo (%zu bytes)", pkt_len);

    LOG_DBG("HTTP POST a %s (%zu bytes)...", st->inform_url, pkt_len);
    unsigned char *resp_body=NULL;
    size_t resp_len=0;
    int status=http_post(st->inform_url,"application/x-binary-data",pkt,pkt_len,&resp_body,&resp_len);
    free(pkt);

    LOG_DBG("HTTP status=%d resp_len=%zu", status, resp_len);

    if (status<0) { snprintf(err_out,127,"HTTP connect failed"); LOG_DBG("HTTP FALLO (connect)"); return -1; }
    if (status!=200) { snprintf(err_out,127,"HTTP %d",status); LOG_DBG("HTTP status inesperado %d",status); free(resp_body); return -1; }

    if (!resp_body||resp_len==0) {
        LOG_DBG("respuesta vacia (noop implicito)");
        free(resp_body); return 0;
    }
    LOG_DBG("descifrando respuesta (%zu bytes)...", resp_len);
    DBG_HEX("respuesta raw (primeros bytes)", resp_body, resp_len<32?(int)resp_len:32);

    char *resp_json=parse_packet(resp_body,resp_len,key_hex);
    free(resp_body);
    if (!resp_json) { snprintf(err_out,127,"parse_packet failed"); LOG_DBG("parse_packet FALLO"); return -1; }
    LOG_TRACE("JSON respuesta: %s", resp_json);

    struct json_object *resp_obj=json_tokener_parse(resp_json);
    free(resp_json);
    if (!resp_obj) { snprintf(err_out,127,"JSON parse failed"); LOG_DBG("json_tokener_parse FALLO"); return -1; }
    ...
    if (strcmp(action,"noop")!=0) {
        ...
        LOG_DBG("accion ejecutada: %s", action);
    } else {
        LOG_DBG("accion: noop (sin cambios)");
    }
    return 0;
}
